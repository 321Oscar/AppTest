        ...
        private void BtnSaveChannel_Click(object sender, EventArgs e)
        {
            ...
            if(projectItem.Form.Count > 0)
            {
                ...
                foreach (var item in projectItem.Form)
                {
                    ...
                    {
                        for (int i = 0; i < item.Singals.Signal.Count; i++)
                        {
                            var newSignal = signals.Signal.Find(x => x.SignalName == item.Singals.Signal[i].SignalName && x.MessageID == item.Singals.Signal[i].MessageID);
                            if (newSignal != null)
                            {
                                item.Singals.Signal[i] = newSignal;
                            }
                        }
                    }
                }
