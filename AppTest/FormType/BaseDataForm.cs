    ...
    public partial class BaseDataForm : MetroForm
    {
        ...
        protected virtual void RegisterOrUnRegisterDataRecieve(bool get)
        {
            if (USBCanManager.Instance.Exist(OwnerProject))
            {
                if (get)
                {
                    USBCanManager.Instance.USBCans[OwnerProject].Register(OnDataRecieveEvent, CanChannel);
                }
                else
                {
                    USBCanManager.Instance.USBCans[OwnerProject].UnRegister(OnDataRecieveEvent, CanChannel);
                }
            }
        ...
        {
            CurrentIDs = Signals.Signal.Select(x => int.Parse(x.MessageID, System.Globalization.NumberStyles.HexNumber)).Distinct().ToList();
        ...
        protected virtual async void OnDataRecieveEvent(object sender, UsbCan.CANDataRecieveEventArgs args)
        {
            try
            {
                ...
                CAN_msg[] rx_mails = args.can_msgs;
                if (null == rx_mails)
        ...
        {
            AddNewForm editForm = new AddNewForm(this.OwnerProject, this.OwnerProject.Form.Find(x => x.Name == this.Name));
