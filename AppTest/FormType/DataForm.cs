...
namespace AppTest.FormType
{
    public partial class DataForm : BaseDataForm
    {
        ...
        public DataForm(FormType formType):this()
        {
        ...
        protected override void InitSignalUC()
        {
            ...
            metroComboBox_Signal.DataSource = Signals.Signal;
            metroComboBox_Signal.DisplayMember = "SignalName";

            BindingList<DBCSignal> bs = new BindingList<DBCSignal>(Signals.Signal);
        ...
        private void btnSetOrSend_Click(object sender, EventArgs e)
        {
            ...
            if (this.FormType == FormType.Set)//Set  发送数据
            {
                ...
                {
                    Dictionary<DBCSignal, string> keyValues = new Dictionary<DBCSignal, string>();
                    var signals = this.Signals.Signal.Where(x => x.MessageID == CurrentIDs[i].ToString("X"));
        ...
        private void btnChangeValue_Click(object sender, EventArgs e)
        {
            if (metroComboBox_Signal.SelectedIndex != -1)
            {
                ...
                this.btnSetOrSend_Click(null, null);
            }
        }
        ...
        private void AddSendThread()
        {
            for (int i = 0; i < CurrentIDs.Count; i++)
            {
                int time = this.Signals.Signal.Where(x => x.MessageID == CurrentIDs[i].ToString("X")).ToList()[0].CycleTime;
                if (time == 0)
                {
        ...
        private void AutoSendThread(object MessageID)
        {
            ...
            int time = this.Signals.Signal.Where(x => x.MessageID == messageID.ToString("X")).ToList()[0].CycleTime;
        ...
        private void Send(int messageID)
        {
            ...
            try
            {
                ...
                var signals = this.Signals.Signal.Where(x => x.MessageID == messageID.ToString("X"));
                foreach (var item in signals)
                {
