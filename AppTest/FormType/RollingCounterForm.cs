...
namespace AppTest.FormType
{
    ...
    public partial class RollingCounterForm : BaseDataForm
    {
        ...
        {
            if (null == Signals || Signals.Signal.Count == 0)
                return;
            ...
            for (int i = 0; i < Signals.Signal.Count; i++)
            {
                SignalInfoUC signalInfoUS = new SignalInfoUC(Signals.Signal[i], true);
                signalInfoUS.Name = Signals.Signal[i].SignalName;
                minHeigt = signalInfoUS.Height;
                //signalInfoUS.SetData("0");
                signalInfoUS.Dock = DockStyle.Top;
                SignalUC.Add(Signals.Signal[i].SignalName, signalInfoUS);

                //保留原有信号的值
                if (oldSignals.ContainsKey(Signals.Signal[i].SignalName))
                {
                    signalInfoUS.SignalValue = oldSignals[Signals.Signal[i].SignalName].SignalValue;
                }

                GroupBox groupBox = gb.Find(x => x.Name == Signals.Signal[i].MessageID + ";" + Signals.Signal[i].CycleTime);

                if (groupBox == null)
                {
                    groupBox = new GroupBox()
                    {
                        Name = Signals.Signal[i].MessageID + ";" + Signals.Signal[i].CycleTime,
                        Text = "ID:" + Signals.Signal[i].MessageID,
                        AutoSize = true,
            ...
            metroComboBox_Signals.DataSource = Signals.Signal;
        ...
        private void Send(object groupbox)
        {
            ...
            Dictionary<DBCSignal, string> keyValues = new Dictionary<DBCSignal, string>(signalEqualityComarer);
