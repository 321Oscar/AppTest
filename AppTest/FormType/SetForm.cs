...
namespace AppTest.FormType
{
    ...
    public partial class SetForm : BaseDataForm
    {
        ...
        private void AddSignalToPanel(out int minHeight)
        {
            ...
            for (int i = Signals.Signal.Count-1; i >= 0; i--)
            {
                SignalInfoUC signalInfoUS = new SignalInfoUC(Signals.Signal[i], false);
                ...
                GroupBox groupBox = gb.Find(x => x.Name == Signals.Signal[i].MessageID);

                if (groupBox == null)
                {
                    groupBox = new GroupBox()
                    {
                        Name = Signals.Signal[i].MessageID,
                    ...
                    if (lfp is FlowLayoutPanel)
                    {
                        foreach (var item in lfp.Controls)
                        {
                            if (item is SignalInfoUC uS)
                            {
                                ...
                                this.Signals.Signal.Find(x => x == uS.Signal).StrValue = uS.SignalValue;
                            }
                        }
                    }
                }
