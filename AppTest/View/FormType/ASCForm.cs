    ...
    public partial class ASCForm : Form
    {
        ...
        private void metroComboBoxDBCFiles_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(metroComboBoxDBCFiles.SelectedIndex > -1)
            {
                ...
                try
                {
                    ...
                    foreach (var signal in BaseProtocol.GetSingalsByProtocol((int)ProtocolType.DBC, new string[] { fileName }))
                    {
                        if (signal is DBCSignal)
                            allSingals.SignalList.Add((DBCSignal)signal);
                    }
                    //allSingals.SignalList = BaseSignal.AutoCopyList<BaseSignal,DBCSignal>( BaseProtocol.GetSingalsByProtocol((int)ProtocolType.DBC, new string[] { fileName }));
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                var signals = allSingals.SignalList.FindAll(x => x.MessageID == signal.CAN_Msg.cid.ToString("X"));
                DBCSignals singals = new DBCSignals();
